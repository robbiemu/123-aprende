...
const request = async (method = 'GET', url = '', params = {}, token) => {
  const uri = url
    .replace(/!/g, '%21')
    .replace(/'/g, '%27')
    .replace(/\(/g, '%28')
  ...
  const options = {
    method,
    headers: {
      Authorization: token || Util.createHeaderString(params)
    }
  }

  if (token) options.body = JSON.stringify(params)

  console.log('---twitter requesting', uri, options)

  const result = await fetch(uri, options).then(response => {
    console.log('---twitter responding', response)
