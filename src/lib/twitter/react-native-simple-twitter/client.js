...
export const streamURL = 'https://stream.twitter.com/1.1'
export const apiURL = 'https://api.twitter.com/1.1'
...
class Client {
  constructor () {
    ...
    this.api = async (
      ...
      this.TokenRequestHeaderParams = await Util.createSignature(
        this.TokenRequestHeaderParams,
        apiMethod,
        host + hostEndpoint,
        this.ConsumerSecret,
      ...
      const url =
        host +
        (params && !token
          ? `${hostEndpoint}?${Util.encodeParamsToString(params)}`
          : hostEndpoint)

      const result = await Request(
        apiMethod,
        url,
        token ? params : this.TokenRequestHeaderParams,
        token
      )
      return result
    }
